...
void main() {
  group('SyncController (architecture §4.13)', () {
    test('stamps completedAt on the initial sync run', () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final container = ProviderContainer(overrides: [
        authRepositoryProvider.overrideWithValue(_SignedInAuth('u1')),
        syncEngineProvider.overrideWithValue(
            SyncEngine(db: db, remote: _FakeRemote())),
      ]);
      addTearDown(container.dispose);

      final result = await container.read(syncControllerProvider.future);
      expect(result.completedAt, isNotNull);
      expect(result.pushed, 0);
      expect(result.pulled, 0);
    });

    test('syncNow runs another pass and refreshes the last-run result',
        () async {
      ...
      expect(container.read(syncControllerProvider).valueOrNull!.completedAt,
          isNotNull);
    });
    ...
    test('syncNow is a no-op when signed out', () async {
      ...
      addTearDown(db.close);
      final container = ProviderContainer(overrides: [
        authRepositoryProvider.overrideWithValue(_SignedOutAuth()),
        syncEngineProvider.overrideWithValue(
            SyncEngine(db: db, remote: _FakeRemote())),
      ]);
      addTearDown(container.dispose);

      final idle = await container.read(syncControllerProvider.future);
      expect(idle.completedAt, isNull);
      await container.read(syncControllerProvider.notifier).syncNow();
      expect(container.read(syncControllerProvider).valueOrNull!.completedAt,
          isNull);
    });
